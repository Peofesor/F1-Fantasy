...
- **1 backmarker slot** — a free pick, any driver, no tier restriction. It is **scored in reverse**: points equal to finishing position, so a worse finish pays more. It is still budget-costed to fill, which is what makes choosing *which* backmarker a real decision rather than free upside.
  - It paid **cost cap** rather than points **up to and including 2026 round 14**; from round 15 it pays points. The amount is identical either way — only the currency changed. Two currencies for one roster left the only slot on the round card that could not be compared against an opponent's: nine scores and a dash.
  - **Earlier rounds keep the cap they paid.** Re-scoring them under the new rule stripped 262 of cap that members had already spent on rosters and bets that still stand, putting every bank in the league under water — one of them by 213 — and blocking betting outright, since a stake cannot exceed the bank. A settled round is a record, the same principle the fixture list and the bet ledger already follow, so the rule changes at a round boundary rather than for the season. `BACKMARKER_POINTS_FROM` in `scoring.ts` is the boundary, and `RoundFacts.backmarkerScoresPoints` carries it into the scorer so the scorer itself stays free of dates.
...
- **The backmarker cannot be captained.** It is scored for finishing badly, so an armband on it would pay double for the worst car on the grid.
...
**The backmarker slot is scored in reverse, like the reverse constructor** (see §4): one point per place from the front, capped at last place, so a retirement is worth exactly a last place and no more. It paid that amount as cost cap rather than points before 2026 round 15. A disqualification pays nothing — it is a removal from the classification rather than a bad result, and usually the team's own doing.
...
- **Odds are priced per selection, not per market.** A fixed price per market cannot hold when the player picks who it applies to. Measured across 2025–26, every market paid more than it was worth to somebody: "Reaches Q3" at 1.6 was 95% certain on Norris, and "Does not finish" at 4.0 came in 92% of the time on Bottas — an expected return of **3.69× the stake**. Eight of nine markets were profitable at listed odds and all nine were once the pre-qualifying bonus applied. Prices now come from how often that selection has actually produced that outcome over two seasons, times a **0.9 house margin**, with a smoothing term so a 13-for-13 record does not price at zero. The odds agreed are stored on the bet, since a price that moves with form cannot be re-derived at settlement.
- **The pre-qualifying bonus dropped from 1.5 to 1.1.** Against fair prices a half-again multiplier simply undid the margin. Worst case across the field went from 5.54× the stake to 1.07×.
- **The 1.1 is folded into the quoted price** rather than added at settlement. Once betting closed at qualifying every bet carried it, and a multiplier that always applies is not a bonus — it is part of the price. The paddock was quoting one number and paying another, leaving the player to do the arithmetic. Payouts are unchanged: a bet still returns `stake × (1 + odds)` where the stored odds now include the premium, and existing rows were scaled to match. It also makes the real edge visible, which the separate bonus hid: expected return is `0.99 − 0.1 × rate` per unit staked, so the house keeps 10% on an even-money pick and about 1% on a long shot. If that is too thin, the lever is `HOUSE_MARGIN`, not a second multiplier.
...
- **The board is grouped by the session that settles it** — qualifying, sprint, race, in the order the weekend runs. Fourteen markets in one flat list, ordered by price, meant reading all of them to find the two about Saturday: the price ordering that helps inside a group is exactly what scatters the groups. Pole position was added at the same time, and settles on the qualifying classification rather than the grid — a penalty applied afterwards moves where a car starts, not who won the pole.
- **Betting closes when qualifying starts**, the same deadline as the roster. Two timing tiers were tried — pre-qualifying at the bonus, pre-race at listed odds — and gave the weekend two deadlines to remember, the second of them in the middle of a Sunday. One deadline per round is the rule now, so every new bet is a pre-qualifying one; `pre_race` survives only on rows taken while the market ran on, and they settle on the terms they were struck at.
