 ...
 * backmarker slot pays cost cap instead (see `backmarkerPoints`), and the
...
/**
 * The round from which the backmarker slot pays points instead of cost cap.
 *
 * The slot was always scored in reverse; only the currency changed. Rounds
 * before this keep the cap they paid at the time: members spent that cap on
 * rosters and bets that still stand, and rewriting it retroactively left every
 * bank in the league hundreds under water for money legitimately earned.
 *
 * So the rule changes at a round boundary rather than for the season. A
 * settled round is a record and is left alone — the same principle the fixture
 * list and the bet ledger already follow.
 */
export const BACKMARKER_POINTS_FROM = { season: 2026, round: 15 } as const;

/** Whether this round's backmarker pays points rather than cost cap. */
export function backmarkerScoresPoints(season: number, round: number): boolean {
  if (season !== BACKMARKER_POINTS_FROM.season) {
    return season > BACKMARKER_POINTS_FROM.season;
  }
  return round >= BACKMARKER_POINTS_FROM.round;
}

/**
 * What the backmarker slot pays — points from `BACKMARKER_POINTS_FROM`, cost
 * cap before it. The amount is the same either way; only what it buys differs.
 *
 * A worse finish pays more, which is the whole point of the slot.
 *
 ...
 */
export function backmarkerPoints(
  finishPosition: number | null,
  classification: FinishClassification,
